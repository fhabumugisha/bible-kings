...
import { generateCardQuiz } from '@/lib/quiz'
...
export function QuizModal({ kingId, isOpen, onClose }: QuizModalProps) {
  ...
  const questions = useMemo(
    () => (isOpen ? generateCardQuiz(kingId) : []),
    [kingId, isOpen]
  )
  ...
  useEffect(() => {
    const resetKey = `${kingId}-${isOpen}`
    if (isOpen && questions.length > 0 && lastResetKey.current !== resetKey) {
      lastResetKey.current = resetKey
      quiz.reset()
    }
  }, [isOpen, kingId, questions.length, quiz]);

  useEffect(() => {
    if (quiz.isComplete) {
      recordScore(`card-${kingId}`, quiz.score, quiz.totalQuestions);
    }
  ...
  const previousBest = quizScores[`card-${kingId}`]
