...
const previewProphets = PROPHETS.filter((p) => p.impact === 5)
const rotations = [-3, 2, -1]

export default function ProphetHero() {
  const prophetsViewed = useProgressStore((state) => state.prophetsViewed)
  const [hydrated, setHydrated] = useState(false)
  useEffect(() => setHydrated(true), [])
  const progress = hydrated
    ? Math.round((prophetsViewed.length / PROPHETS.length) * 100)
    : 0

  const handleExploreClick = () => {
    const cardGrid = document.getElementById('prophet-grid')
    if (cardGrid) {
  ...
  const PreviewCards = ({ className }: { className?: string }) => {
    ...
    return (
      ...
      >
        {previewProphets.slice(0, 3).map((prophet, i) => (
          <motion.div
            key={prophet.id}
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:gap-12">
          ...
          <div className="lg:w-[55%]">
            ...
            <p className="font-inter font-light text-white/70 text-lg md:text-xl mb-5 max-w-2xl">
              Découvrez les 23 prophètes qui ont marqué l&apos;histoire d&apos;Israël à travers
